...
(function($) {
    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        var defaults = { // start off with some default settings
                ...
                containerClass:    'filter-table act-button-collapse-all',      // class to apply to the container
                ...
                minRows:           0,                   // don't show the filter on tables with at least this number of rows
                placeholder:       'Search Customers',  // HTML5 placeholder text for the filter field
        ...
        var doFiltering = function(table, q) { // handle the actual table filtering
                ...
                if (q === '' || q.length < settings.minChars) { // if the filtering query is blank or the number of chars is less than the minChars option
                    tbody.find('th').show().addClass(settings.visibleClass); // show all rows
                    tbody.find('th').removeClass(settings.highlightClass); // remove the row highlight from all cells
                    if (settings.hideTFootOnFilter) { // show footer if the setting was specified
                        table.find('tfoot').show();
                    }
                } else { // if the filter query is not blank
                    var all_tds = tbody.find('th');
                    tbody.find('th').hide().removeClass(settings.visibleClass); // hide all rows, assuming none were found
                    ...
                    if (settings.ignoreColumns.length) {
                        ...
                        tds.each(function() {
                            ...
                            if ($.inArray(col, settings.ignoreColumns) === -1) {
                                ...
                                t.addClass(settings.highlightClass).closest('th').show().addClass(settings.visibleClass);
                            }
                        });
                    } else {
                        if (settings.ignoreClass) {
                        ...
                        all_tds.filter(':'+settings.filterExpression+'("'+q+'")').addClass(settings.highlightClass).closest('th').show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
                    }
                }
        ...
        return this.each(function() {
            ...
            if (t[0].nodeName === 'TABLE' && tbody.length > 0 && (settings.minRows === 0 || (settings.minRows > 0 && tbody.find('th').length >= settings.minRows)) && !t.prev().hasClass(settings.containerClass)) { // only if object is a table and there's a tbody and at least minRows trs and hasn't already had a filter added
