...
(function($) {
    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        var defaults = { // start off with some default settings
                ...
                containerClass:    'filter-migrations',      // class to apply to the container
                ...
                bwInputSelector:     null,                // use the element with this selector for the filter input field instead of creating one
                inputName:         '',                  // name of filter input field
                bwInputName:         '',                  // name of filter input field
                inputType:         'search',            // tag name of the filter input tag
                bwInputType:         'search',            // tag name of the filter input tag
                label:             'Filter:',           // text to precede the filter input tag
                bwLabel:             'Exclusion Filter:',           // text to precede the filter input tag
                ...
                bwPlaceholder:       'Exclude from Table', // HTML5 placeholder text for the filter field
        ...
        var doBackwardsFiltering = function(table, q) { // handle the actual table filtering
                var tbody = table.find('tbody'); // cache the tbody element
                if (q === '' || q.length < settings.minChars) { // if the filtering query is blank or the number of chars is less than the minChars option
                    tbody.find('tr').show().addClass(settings.visibleClass); // show all rows
                    tbody.find('td').removeClass(settings.highlightClass); // remove the row highlight from all cells
                    ...
                    var all_tds = tbody.find('td');
                    tbody.find('tr').show().addClass(settings.visibleClass); // hide all rows, assuming none were found
                    all_tds.removeClass(settings.highlightClass); // remove previous highlights
                    if (settings.hideTFootOnFilter) { // hide footer if the setting was specified
                        table.find('tfoot').hide();
                    }
                    if (settings.ignoreColumns.length) {
                        ...
                        all_tds.filter(':'+settings.filterExpression+'("'+q+'")').removeClass(settings.highlightClass).closest('tr').hide().removeClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
                    }
                }
                if (settings.callback) { // call the callback function
                    settings.callback(q, table);
        ...
        return this.each(function() {
            var t = $(this), // cache the table
                ...
                bwcontainer = null, // placeholder for the filter field container DOM node
                ...
                created_bwfilter = true; // was the filter created or chosen from an existing element?
            if (t[0].nodeName === 'TABLE' && tbody.length > 0 && (settings.minRows === 0 || (settings.minRows > 0 && tbody.find('tr').length >= settings.minRows)) && !t.prev().hasClass(settings.containerClass)) { // only if object is a table and there's a tbody and at least minRows trs and hasn't already had a filter added
                if (settings.inputSelector && $(settings.inputSelector).length === 1) { // use a single existing field as the filter input field
                    ...
                    backwardsfilter = $('<input type="'+settings.inputType+'" placeholder="'+settings.placeholder+'" name="'+settings.inputName+'" />'); // build the filter field
                ...
                if (settings.bwInputSelector && $(settings.bwInputSelector).length === 1) { // use a single existing field as the filter input field
                    bwfilter = $(settings.bwInputSelector);
                    bwcontainer = bwfilter.parent(); // container to hold the quick list options
                    created_bwfilter = false;
                } else { // create the filter input field (and container)
                    bwcontainer = $('<'+settings.containerTag+' />'); // build the container tag for the filter field
                    if (settings.containerClass !== '') { // add any classes that need to be added
                        bwcontainer.addClass(settings.containerClass);
                    }
                    bwcontainer.prepend(settings.bwLabel+' '); // add the label for the filter field
                    bwfilter = $('<input type="'+settings.bwInputType+'" placeholder="'+settings.bwPlaceholder+'" name="'+settings.bwInputName+'" />'); // build the filter field
                    if (settings.preventReturnKey) { // prevent return in the filter field from submitting any forms
                        bwfilter.on('keydown', function(ev) {
                            if ((ev.keyCode || ev.which) === 13) {
                                ev.preventDefault();
                                return false;
                            }
                ...
                if ($.fn.bindWithDelay) { // does bindWithDelay() exist?
                    ...
                    bwfilter.bindWithDelay('keyup', function() { // bind doFiltering() to keyup (delayed)
                        filter.val("");
                        doBackwardsFiltering(t, $(this).val());
                    }, 200);
                    ...
                    bwfilter.bind('keyup', function() { // bind doFiltering() to keyup
                        filter.val("");
                        doBackwardsFiltering(t, $(this).val());
                    });
                } // keyup binding block
                ...
                bwfilter.bind('click search input paste blur', function() { // bind doFiltering() to additional events
                    filter.val("");
                    doBackwardsFiltering(t, $(this).val());
                });
                if (created_filter) { // add the filter field to the container if it was created by the plugin
                    container.append(filter);
                    bwcontainer.append(bwfilter);
                }
                ...
                if (created_filter) { // add the filter field and quick list container to just before the table if it was created by the plugin
                    $('#table-wrapper').before(container);
                    $('#table-wrapper').before(bwcontainer);
                }
